...
class ExpressionWriter {
    ...
    fun processAction(action: CalculatorAction) {
        when (action) {
            ...
            is CalculatorAction.Operator -> {
                if (canEnterOperation(action.value))
                    expression += action.value.symbol
            }
    ...
    private fun canEnterOperation(operation: Operation): Boolean {
        ...
        return expression.isNotEmpty() || expression.last() in ")123456789"
    }

    private fun canEnterDecimal(): Boolean {
        if (expression.isEmpty() || expression.last() in "$operationSymbols.()")
            return false
        return !expression.takeLastWhile { it in "0123456789." }.contains(".")
    }
